...
function updateCertificateDetails(certificate) {
    document.getElementById('studentName').textContent = certificate.studentName;
    document.getElementById('degree').textContent = certificate.degree;
    document.getElementById('institution').textContent = certificate.institution;
    document.getElementById('graduationDate').textContent = certificate.graduationDate;
    document.getElementById('ipfsHash').textContent = certificate.ipfsHash;
    
    const statusElement = document.getElementById('certificateStatus');
    statusElement.textContent = certificate.isValid ? 'Valid' : 'Invalid';
    statusElement.className = `status-badge ${certificate.isValid ? 'status-valid' : 'status-invalid'}`;
    
    certificateImage.src = `https://ipfs.io/ipfs/${certificate.ipfsHash}`;
    certificateImage.style.display = 'block';
    
    certificateDetails.classList.add('show');
}
...
verifyForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (!isValid) {
            showToast('Certificate is invalid or does not exist', 'error');
            return;
        }
        
        // Get certificate details
        const certificate = await contract.getCertificate(tokenId);
        updateCertificateDetails(certificate);
        ...
        verifyBtn.innerHTML = '<i class="fas fa-search"></i> Verify Certificate';
    }
});
